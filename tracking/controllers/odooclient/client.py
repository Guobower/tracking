...
class OdooClient(object):
    ...
    def Read(self, model, document_ids, fields=False, context=None):
        ...
        if not context:
            context = {}
        if type(document_ids) not in (int, list, tuple):
            msg = "Invalid ids `type` {ids}. Ids should be on type `int`, \
                                    `long`, `list` or 'tuple'.".format(ids=document_ids)
            return (False, msg)
        service = connection.Connection(self._url)
        response = service.Model(self._db, self._uid, self._password, model, 'read', document_ids, {'fields': fields})
        return response
    ...
    def UnlinkWorkflow(self, model, document_ids, context=None):
        ...
        if not context: context = {}
        if type(document_ids) not in (int,  list, tuple):
            raise Exception("Document Ids expected to be in int,  list or tuple format.")
        if type(document_ids) in (int):
            document_ids = [document_ids]
        service = connection.Connection(self._url)
        response = service.Model(self._db, self._uid, self._password, model, 
